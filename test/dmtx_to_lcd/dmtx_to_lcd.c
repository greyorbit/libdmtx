...
LCD display direction
 ...
 ----->
*/
int dmtxToLcd(unsigned char *str, DmtxSymbolSize size, unsigned char *lcdBuf) {
    int width, height;
    DmtxEncode *enc;
    ...
    if (width == DmtxUndefined) {
        return -1;
    }
    if (height == DmtxUndefined) {
        return -1;
    }
    for (int h = 0; h < height; h++) {
        for (int w = 0; w < width; w++) {
            if (enc->image->pxl[w + (h * width)] == 0) {
                printf("#");
            } else {
                printf(" ");
            }
...
int main()
{
    unsigned char lcdBuf[100];
    dmtxToLcd("AABBCC112233", DmtxSymbol16x16, lcdBuf);
    for (int h = 0; h < 2; h++)
    {
        for (int w = 0; w < 16; w++)
        {
            printf("0x%02x ", lcdBuf[w + h * 16]);
        }
        printf("\r\n");
    }
    return 0;
